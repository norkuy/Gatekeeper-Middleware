...
function gateKeeper(req, res, next) {
  ...
  let userFound;
  const userPassHeader = req.header('x-username-and-password');
  const userPassObj = queryString.parse(userPassHeader);
  
  if (userPassHeader) {
    userFound = USERS.find(user => user.userName === userPassObj.user && user.password === userPassObj.pass);
  }
